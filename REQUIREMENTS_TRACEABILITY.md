...
| 30 | Raw LogQL gated: Loki-only, off by default, config-enabled, bounded | 9.1 | E | Partial | Phase D groundwork: LokiProperties#rawLogQlEnabled (default false, config-gated); LokiLogSource#capabilities() reports it honestly, never faked (LokiLogSourceTest#capabilitiesReflectConfiguredFlagsHonestlyWhen*). Actual raw-LogQL execution mode is Phase E's job, not yet built. |
