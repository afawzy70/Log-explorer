...
- Never assumes uniform capability across sources — the backend
  ...
  truthfully.
- Masks five protected fields (CIF, Username, Customer ID, Device ID,
  Device IP) server-side, before serialization, by default, with no
  client-side reconstruction and no per-row "reveal" action anywhere in
  the product — this is a security invariant, not a configurable nicety
  that can be quietly weakened.
...
**Implemented and verified today (the "current baseline," frozen at the
git tag `functional-baseline-pre-ux-redesign`):** search with time
...
Docker-specific and OpenShift-specific settings.
