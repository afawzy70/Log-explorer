...
# logexp-test-orders starts at VERSION=v1 (set by deploy.sh), generates
# some traffic, then this script performs a normal rolling update to
...
echo "Current logexp-test-orders pods (v1):"
oc get pods -n "${TESTBED_NAMESPACE}" -l app=logexp-test-orders

echo "Rolling logexp-test-orders to VERSION=v2 ..."
oc set env deployment/logexp-test-orders -n "${TESTBED_NAMESPACE}" VERSION=v2

echo "Watching rollout (Ctrl+C to stop watching once you've observed the overlap in Log Explorer) ..."
oc rollout status "deployment/logexp-test-orders" -n "${TESTBED_NAMESPACE}" --timeout=180s &
WATCH_PID=$!
...
wait "${WATCH_PID}" || true
echo "Current logexp-test-orders pods (post-rollout):"
oc get pods -n "${TESTBED_NAMESPACE}" -l app=logexp-test-orders
