...
public record OpenShiftScope(
  ...
  public static final OpenShiftScope EMPTY = new OpenShiftScope(List.of(), null, List.of(), null, List.of(), null);

  /**
   * Replaces the discovered workload list. If the currently-selected
   * workload is no longer in it, the selection (and everything below it)
   * is cleared truthfully rather than kept pointing at a workload that
   * disappeared (OS-1B §14 "Workload disappears").
   */
  public OpenShiftScope withWorkloads(List<WorkloadSummary> newWorkloads) {
    boolean stillPresent = selectedWorkload != null
        && newWorkloads.stream().anyMatch(w -> w.ref().equals(selectedWorkload));
    if (stillPresent) {
      return new OpenShiftScope(List.copyOf(newWorkloads), selectedWorkload, pods, selectedPod, containers,
          selectedContainer);
    }
    return new OpenShiftScope(List.copyOf(newWorkloads), null, List.of(), null, List.of(), null);
  }
  ...
  public OpenShiftScope withSelectedWorkload(WorkloadRef ref) {
    return new OpenShiftScope(workloads, ref, List.of(), null, List.of(), null);
  }
  ...
  public OpenShiftScope withPods(List<PodSummary> newPods) {
    ...
    if (stillPresent) {
      return new OpenShiftScope(workloads, selectedWorkload, List.copyOf(newPods), selectedPod, containers,
          selectedContainer);
    }
    return new OpenShiftScope(workloads, selectedWorkload, List.copyOf(newPods), null, List.of(), null);
  }
  ...
  public OpenShiftScope withSelectedPod(String podName) {
    return new OpenShiftScope(workloads, selectedWorkload, pods, podName, List.of(), null);
  }
  ...
  public OpenShiftScope withContainers(List<String> newContainers) {
    ...
    return new OpenShiftScope(workloads, selectedWorkload, pods, selectedPod, List.copyOf(newContainers), kept);
  }

  public OpenShiftScope withSelectedContainer(String containerName) {
    return new OpenShiftScope(workloads, selectedWorkload, pods, selectedPod, containers, containerName);
  }
