...
- Node 20 (`actions/setup-node@v4`) — matches `Dockerfile`'s own `FROM node:20-alpine AS frontend-build` stage, the repo's documented Node version.
- Runs from `frontend/`.
...
- Java 21 + Node 20 (same setup as the other two jobs — this job needs both toolchains, since it runs the real backend *and* the real frontend dev server).
...
- **Backend**: started via `SPRING_PROFILES_ACTIVE=dev nohup ./mvnw --batch-mode --quiet --offline spring-boot:run &`, backgrounded, PID captured. A direct `spring-boot:run` goal invocation (not a lifecycle phase) never triggers the `test` phase on its own, so this does not re-run the suite the `Backend` job already ran — it only needs a live instance of the real backend for the browser to talk to.
