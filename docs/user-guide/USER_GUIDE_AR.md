...
- حقل **Severity** — يفتح لوحة فيها إجراءان سريعان **All** و**Errors
  only**، بالإضافة إلى Trace/Debug/Info/Warn/Error و**Unknown** كمستويات
  يمكن تفعيل كل منها بشكل مستقل (كلها مفعّلة افتراضيًا).
...
- **Severity** — إجراءان سريعان **All** أو **Errors only**، أو اختيار أي
  توليفة من Trace/Debug/Info/Warn/Error بشكل مستقل. **Unknown** مستوى
  سادس منفصل: حدث سليم البنية لم يحمل مصدره (أو تعيين الحقول الحالي) أي
  قيمة لمستوى الخطورة إطلاقًا — وهذا مختلف عن السطر التالف/غير القابل
  للتحليل، الذي يبقى ظاهرًا دائمًا بصرف النظر عن هذا الفلتر. فعّله لرؤية
  هذه الفئة وحدها، أو ألغِ تفعيله لاستبعادها — وهو مُفعَّل عندما تكون كل
  المستويات مُفعَّلة (الوضع الافتراضي)، تمامًا كأي مستوى آخر.
