...
> الابتدائية لمربّعات الاختيار الخمسة هي ما تغيّر. مثل كل إعداد آخر في
> Log Explorer، لا تُحفَظ هذه السياسة على القرص — فإعادة تشغيل الخادم
...
2. **تشغيل فحص سريع للمخطّط (Quick Schema Scan)** — اضغط لفحص دفعة
   صغيرة ومحدودة من الأحداث الحقيقية والحديثة مباشرة من مصدرك (ليس
   بحثًا كاملًا). على خلاف عيّنة واحدة، يبحث الفحص عمدًا عن *التنوّع* —
   مستويات خطورة مختلفة (INFO/WARN/ERROR/...) وأشكال JSON مختلفة — لأن
   أنواع الأحداث المختلفة في المصدر نفسه غالبًا ما تحمل حقولًا مختلفة
   (حقل تتبّع الخطأ في حدث ERROR، مثلًا، قد لا يظهر أبدًا في حدث INFO).
   الفحص محدود (حتى 200 حدث افتراضيًا، وبضع ثوانٍ، وبضعة ميغابايتات)
   ويُخبرك بصدق دائمًا إن وصل إلى أحد هذه الحدود قبل أن يُتمّ الفحص.
3. **مراجعة عيّنات الأحداث الأصلية (Original Event Samples)** — مجموعة
   صغيرة من الأحداث الحقيقية التي رآها الفحص فعلًا، معروضة تمامًا كما
   أرسلها مصدرك، قبل أن يغيّر Log Explorer أي شيء فيها. هذا مختلف عن
   "Canonical Event JSON" في الفاحص (§11) — ذاك يعرض نسخة Log Explorer
   المُطبَّعة من الحدث؛ أمّا هذا فيعرض الأصل دون تغيير.
4. **مراجعة مخطّط المصدر المُكتشَف (Discovered Source Schema)** — جدول
   بكل مسار JSON عثر عليه الفحص عبر كل الأحداث التي فحصها، مع عدد
   الأحداث التي احتوته ونوع القيمة فيه. أسماء الحقول وتداخلها الحقيقيان
   هما تمامًا كما هي في مصدرك — `cif`، `cifId`، `customer.cif`،
   `mdc.cif` تُعرض جميعها كمسارات مختلفة فعلًا وواضحة، لا تُخلَط
   ببعضها أبدًا. هذا الجدول موصوف بأنه **مُلاحَظ (observed)**، وليس
   كاملًا — فالفحص لا يمكنه سوى الإبلاغ عمّا رآه فعلًا؛ وهو ليس إثباتًا
   رياضيًا بعدم وجود شكل آخر في مصدرك.
5. **ربط الحقول (Map fields)** — لكل حقل أساسي يفهمه Log Explorer
...
6. **التحقّق (Validate)** — شغّل ربطك مقابل العيّنات التي جمعها الفحص.
   سترى، لكل حقل، هل وُجد فعلًا، وما القيمة الحقيقية التي استُخلِصت،
   وهل يحتوي أي مسار على خطأ.
7. **الحفظ (Save)** — يُثبِّت ربطك. لا يمكنك الحفظ إلا بعد تحقّق لم
   يجد أي خطأ في المسارات.
8. **يعود البحث متاحًا من جديد** — إن كان البحث معطَّلًا مؤقّتًا لأن
...
في أي وقت. إعادة الفحص لا تغيّر ربطك المحفوظ أبدًا من تلقاء نفسها —
تُحدِّث فقط عيّنات الأحداث الأصلية ومخطّط المصدر المُكتشَف، و:
