...
5. **Technical / all fields** — كل حقل يحمله الحدث، بما فيها الحقول
   ...
   قائمة طويلة من الحقول؛ وقسم **Canonical Event JSON** القابل للتوسيع
   يعرض السجل الكامل — وهو تمثيل Log Explorer المُحلَّل والمُطبَّع
   للحدث، وليس سطر المصدر الأصلي دون تغيير (راجع §19 إن احتجت السجل
   الأصلي الحقيقي).
...
افتح **Settings → Log Schema & Field Mapping**. الخطوات بالترتيب:

1. **الاتصال (Connect)** — تأكّد من أن المصدر الذي تريد إعداده
   مُختار ومتّصل (Docker، OpenShift، إلخ — راجع أقسامها أعلاه).
2. **جلب عيّنات (Fetch samples)** — اضغط لسحب دفعة صغيرة ومحدودة من
   الأحداث الحقيقية والحديثة مباشرة من مصدرك (ليس بحثًا كاملًا — فقط
   أمثلة حقيقية كافية لرؤية أسماء حقولك الفعلية).
3. **فحص JSON المصدر الأصلي (Original Source JSON)** — يعرض هذا
   الحدث *الحقيقي* تمامًا كما أرسله مصدرك، قبل أن يغيّر Log Explorer
   أي شيء فيه. هذا مختلف عن "Canonical Event JSON" في الفاحص (§11) —
   ذاك يعرض نسخة Log Explorer المُطبَّعة من الحدث؛ أمّا هذا فيعرض
   الأصل دون تغيير. أسماء الحقول وتداخلها الحقيقيان هما تمامًا كما هي
   في مصدرك — `cif`، `cifId`، `customer.cif`، `mdc.cif` تُعرض جميعها
   كمسارات مختلفة فعلًا وواضحة، لا تُخلَط ببعضها أبدًا.
4. **ربط الحقول الأساسية (Map canonical fields)** — لكل حقل يفهمه Log
   Explorer (CIF، اسم المستخدم، معرّف العميل، Trace ID، اسم الرحلة
   Journey Name، وغيرها)، أخبره بالمسار (أو المسارات، بترتيب أولوية)
   في JSON الحقيقي الذي يجب قراءة القيمة منه. يمكنك إدراج أكثر من
   مسار واحد لحقل ما — يجرّبها Log Explorer بالترتيب الذي تحدّده،
   ويستخدم أوّل مسار يحمل قيمة فعلية لحدث معيّن. هذا بالضبط ما يحلّ
   مشكلة "الحقل موجود لكنه لا يظهر" — إن كانت سجلّاتك تستخدم أحيانًا
   `cif` وأحيانًا `mdc.cif`، أدرج كليهما.
5. **التحقّق (Validate)** — شغّل ربطك مقابل العيّنات الحقيقية التي
   جلبتها. سترى، لكل حقل، هل وُجد فعلًا في بيانات عيّنتك، وما القيمة
   الحقيقية التي استُخلِصت، وهل يحتوي أي مسار كتبته على خطأ.
6. **الحفظ (Save)** — يُثبِّت ربطك. لا يمكنك الحفظ إلا بعد تحقّق لم
   يجد أي خطأ في المسارات.
7. **يعود البحث متاحًا من جديد** — إن كان البحث معطَّلًا مؤقّتًا لأن
   ربطك يحتاج انتباهًا (راجع أدناه)، فإن حفظ ربط صالح يعيد تفعيل
   البحث فورًا.

يتوفّر دائمًا إجراء **Reset to defaults** إن أردت تجاهل تغييراتك
والعودة إلى الربط المُضمَّن.
...
في كل مكان آخر في Log Explorer، تبقى الحقول الخمسة المحميّة (CIF،
اسم المستخدم، معرّف العميل، معرّف الجهاز، عنوان IP للجهاز) مُقنَّعة
افتراضيًا (§15). هذه الشاشة هي الاستثناء المتعمَّد الوحيد: لأنك تحتاج
رؤية أسماء وقيم حقولك *الحقيقية* لإعداد الربط بشكل صحيح، تعرض بيانات
العيّنة ونتائج التحقّق في هذه الشاشة قيمًا حقيقية غير مُقنَّعة. لهذا
من المهم التعامل مع هذه الشاشة بحذر:
